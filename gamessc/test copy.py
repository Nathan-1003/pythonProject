...
#讀取 JavaScript 
with open('/Users/menglungtsai/pythonProject/gamessc/ssc/35824_总和_大.js', 'r') as f:
    js_code = f.read()
...
lottery_sets = generate_lottery_numbers(10)
count_1 = 0
count_minus_1 = 0

for i in range(10):
    result = judge(list(lottery_sets[i]))
    s=sum(lottery_sets[i])
    if result == 1:
        count_1 += 1
        print(f"{Fore.RED}組合 {i + 1}: {lottery_sets[i]} ,總和{s}, 結果: {result}{Style.RESET_ALL}")
    else:
        count_minus_1 += 1

    # 打印結果為-1的組合
    if result == -1:
        print(f"組合 {i + 1}: {lottery_sets[i]} ,總和{s}, 結果: {result}")
